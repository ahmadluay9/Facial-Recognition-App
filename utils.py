...
def attend(PREDICTION_ENDPOINT, EMPLOYEE_DETAILS_ENDPOINT, current_time):
    ...
    if cam_input is not None:
        ...
        # Replace the part of the path you want to change
        base_dir = os.path.dirname(__file__)  # Get the current directory of the script

        # New directory path relative to the current directory
        save_dir = os.path.join(base_dir, 'fastapi', 'Airflow', 'data', 'Face Images', 'Attendance Tracker', 'Attend')

        # save_dir = 'Airflow\data\Face Images\Attendance Tracker\Attend'
        os.makedirs(save_dir, exist_ok=True)
        img_path = os.path.join(save_dir, img_filename)

        # Save the image
        cv2.imwrite(img_path, img_resized)
        st.success(f"Image saved successfully to {img_path}")
...
def leave(PREDICTION_ENDPOINT, EMPLOYEE_DETAILS_ENDPOINT, current_time):
        ...
        if cam_input is not None:
            ...
            # New directory path relative to the current directory
            save_dir = os.path.join(base_dir, 'fastapi', 'Airflow', 'data', 'Face Images', 'Attendance Tracker', 'Leave')
...
def register_st():
    ...
    if cam_input_train is not None:
        ...
        # New directory path relative to the current directory
        save_dir_train = os.path.join(base_dir, 'fastapi', 'Airflow', 'data', 'Face Images', 'Train', f'{imageid}')
    ...
    if cam_input_test is not None:
        ...
        # New directory path relative to the current directory
        save_dir_test = os.path.join(base_dir, 'fastapi', 'Airflow', 'data', 'Face Images', 'Test', f'{imageid}')
